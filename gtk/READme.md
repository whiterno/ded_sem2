# Работа по поиску уязвимостей в ассемблерном коде на архитектуре x86 и написанию бинарного патча для исполняемого файла формата .COM

1) Весь asm код был протестирован на эмуляторe DosBox. Для компиляции, линковки и дебага программ использованы TASM, TLINK, TDEBUGGER. Анализ исполняемого файла проходил в TDEBUGGER.
...
В Dos'е, в отличие от современных операционных систем, нет контроля прав обращения к памяти, контроля системы портов, что позволяет расширить опыт поиска уязвимостей в программе и получить больший контроль над системой.
...
Зачем заниматься этой муторной ручной работой? Смотреть код в TDEBUGGER, искать уязвимости... Можно же просто изменить пару байтов в исполняемом файле и любой пароль будем заходить (даже 0 символов).\

Сделаем изящно, заменим адрес буфера, который используется во время 1-ой попытки ввода, на адрес настоящего пароля. Что бы мы ни написали, программа будет сравнивать строку саму с собой. Всегда будет выдавать True при проверке пароля:

<p align="center"><img src="readme_imgs/3_1.png" width = "800"></p>

<p align="center"><img src="readme_imgs/15.png" width = "800"></p>

Дополнительно к этому напишем графический интерфейс, чтобы посетители кофейни не воспринимали вас за чудика...
...
Добавьте файл, который вы хотите взломать в папку to_crack и напишите свой бинарный патч в качестве функции в patch.cpp (не будет же оно работать по волшебству) \
